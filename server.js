...
async function handleAnalyze(req, res) {
  ...
  const prompt = [
    ...
    "exam에서는 주어와 동사를 따로 쪼개지 마라. 주절은 가능하면 '주어 + 동사구 + 핵심 목적어/보어'까지 한 청크로 묶어라.",
    "exam에서는 조동사구, 현재완료, 수동태, 병렬 동사구를 절대로 분리하지 마라.",
    "exam에서는 너무 잘게 쪼개지 말고, 한 문장을 보통 2~4청크로만 나눠라. 한 청크는 보통 5~14단어 정도가 되게 하라.",
    "exam 나쁜 예: 'many studies / show / that steady practice'. exam 좋은 예: 'many studies show / that steady practice and clear feedback are more important than talent alone'.",
    ...
    "중요: 라벨은 지문의 실제 전개를 그대로 따라가며 골라라. 정해진 순서는 없다.",
    "모든 지문이 통념으로 시작하는 것은 아니다. 글쓴이가 흔한 생각을 소개한 뒤 뒤집을 때만 통념을 사용하라.",
    "첫 문장이 글의 핵심 주장을 바로 제시하면 주제로 시작하고, 사례로 시작하면 예시, 질문으로 시작하면 문제 제기로 시작하라.",
    "각 단계의 text는 해당 문장들의 내용을 구체적으로 요약해야 하며, 지문에 없는 내용을 만들어내지 마라.",
    ...
    "가장 중요한 임무: 주절의 주어와 본동사를 정확히 찾는 것이다. 청킹과 해석은 그 다음이다.",
    "반드시 다음 분석 순서를 지켜라.",
    "1단계(전치사구): 문장의 모든 전치사구를 머릿속으로 괄호 처리하라. 전치사구 안의 명사는 절대 주어가 될 수 없다.",
    "2단계(종속절): 부사절, 명사절, 관계사절을 따로 표시하라. 그 안의 주어와 동사는 주절의 S/V가 아니다.",
    "3단계(주절): 도입 수식어와 종속절을 제외한 뒤 남는 주절을 찾아라.",
    "4단계(주어/본동사): 주절의 정동사(finite verb)를 찾고, 그 동사의 주어를 확정하라.",
    "5단계(청킹): 주어와 본동사를 확정한 다음에만 청킹과 해석을 하라.",
    "절대 금지 사항:",
    "- 전치사구 안의 명사를 주어로 선택하지 마라.",
    "- 분사, 동명사, to부정사를 본동사로 선택하지 마라. 본동사는 시제가 있는 정동사여야 한다.",
    "- renowned, talented, experienced처럼 -ed로 끝나는 형용사를 동사로 착각하지 마라.",
    "- 부사절의 주어와 동사를 주절의 주어와 동사로 선택하지 마라.",
    "- 주어와 본동사를 찾기 전에 청킹하지 마라.",
    "문장마다 syntax 객체를 반드시 넣어라. syntax.subject에는 주절의 정확한 주어 문자열만, syntax.verb에는 주절의 정확한 동사구 문자열만 넣어라.",
    "syntax.subject와 syntax.verb는 반드시 원문 문장에 실제로 등장하는 연속 문자열이어야 한다. 설명을 넣지 말고 원문 그대로만 넣어라.",
    "주어/동사가 애매하면 syntax.subject 또는 syntax.verb를 빈 문자열로 둬라. 절대 추측해서 넣지 마라.",
    "예: 'The challenge this creates is...'에서는 syntax.subject='The challenge this creates', syntax.verb='is'이다.",
    "예: 'as with everything in our universe' 같은 전치사구에는 syntax.subject와 syntax.verb를 넣지 않는다.",
    ...
    "",
    "JSON 형식:",
    '{"passage":{"topicKo":"한글 주제","topicEn":"English topic","keywords":[{"en":"keyword","ko":"키워드"}],"logic":[{"type":"지문의 실제 전개에 맞는 라벨","text":"해당 부분 요약"},{"type":"다음 단계 라벨","text":"해당 부분 요약"}]},"sentences":[{"original":"English sentence","translation":"자연스러운 한국어 해석","syntax":{"subject":"exact main-clause subject","verb":"exact main verb phrase"},"points":{"structure":"주어/동사 등","pronouns":"it = ...","flow":"역접/예시/원인 등"},"chunks":[{"text":"English chunk","translation":"청크 뜻","grammar":"내신 포인트가 있을 때만 짧게","role":"subject"}]}]}',
    "",
    "영어 지문:",
    text,
  ].join("\n");
